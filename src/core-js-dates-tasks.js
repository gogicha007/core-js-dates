...
function getTime(date) {
  const tempDate = new Date(date);
  return tempDate.toString().match(/(\d\d:\d\d:\d\d)/)[0];
}
...
function getNextFriday(date) {
  ...
  let diff = Math.abs(friDay - theDay);
  if (theDay > friDay) {
    diff += ((theDay + friDay) % 7) + 1;
  }
  if (theDay === friDay) {
    diff = 7;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = (end.valueOf() - start.valueOf()) / (1000 * 3600 * 24) + 1;
  return diff;
}
...
function getWeekNumberByDate(date) {
  ...
  firstMonday.setDate(
    firstMonday.getDate() + ((wDay + 7 - firstMonday.getDay()) % 7 || 7)
  );
  const diff = Math.ceil((date - firstMonday + 1) / (24 * 3600 * 1000) / 7);
